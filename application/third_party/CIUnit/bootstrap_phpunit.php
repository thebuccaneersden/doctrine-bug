 ...
 */
$system_path = '/home/' . get_current_user() . '/projects/doctrine-bug/system';
...
define('QSPATH', APPPATH);

require_once APPPATH . '/errors/error_handler.php';
 ...
 *
 * @param string     $dir
 * @param bool|false $root
 */
function includeModels($dir, $root = false)
{
    $files = scandir($dir);
    foreach ($files as $file) {
        ...
        if (is_dir("$dir/$file")) {
            includeModels("$dir/$file");
        }
        if (preg_match("/.+\.php$/", $file)) {
            if (($root) || (trim($file) !== "stylesheet.php")) {
                // There is an entry in the models/ with the same name (stylesheet) so we will ignore it.
...
includeModels(QSPATH . 'models/', true);
